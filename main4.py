...
class EnhancedActivitySimulator:
    def __init__(self, master):
        self.master = master
        master.title("Enhanced Activity Simulator Pro - Input Management Edition")
        ...
        # --- INPUT FIELD MANAGEMENT (NEW) ---
        self.input_widgets = []  # Store all input widgets for enable/disable
        self.setting_widgets = []  # Store setting-specific widgets
        self.checkbox_widgets = []  # Store checkbox widgets
        self.radiobutton_widgets = []  # Store radiobutton widgets
        self.spinbox_widgets = []  # Store spinbox widgets
        self.scale_widgets = []  # Store scale widgets
        self.combobox_widgets = []  # Store combobox widgets
    ...
    def register_input_widget(self, widget, widget_type="general"):
        ...
        # Categorize widgets for specific handling
    ...
    def enable_all_inputs(self):
        ...
        try:
            ...
            for combobox in self.combobox_widgets:
                try:
                    combobox.config(state='readonly')  # Comboboxes use 'readonly' not 'normal'
                except:
                    pass
    ...
    def create_advanced_tab(self, parent):
        ...
        avoid_entry = tk.Entry(app_frame, textvariable=self.avoid_apps, width=60)
    ...
    def create_log_tab(self, parent):
        ...
        self.log_message("🚀 Enhanced Activity Simulator Pro - Input Management Edition initialized!")
        self.log_message("📋 Features: Input field locking, Auto-recovery, Auto-activation, Lock screen support")
        self.log_message("⚠️ Emergency Stop: Move mouse to top-left corner")
        self.log_message("🔒 Input fields will be locked during simulation to prevent conflicts")
    ...
    def start_simulation(self):
        """Start the enhanced simulation with input field locking"""
        if self.is_running.get():
            messagebox.showwarning("Already Running", "Simulation is already active.")
            return
            
        # Lock all input fields FIRST
        ...
        self.log_message("🚀 Starting Enhanced Activity Simulation with Input Management...")
...
INPUT FIELD STATUS
Fields Locked: {'✅ YES' if self.is_running.get() else '❌ NO'}
Lock Reason: {'Simulation Running' if self.is_running.get() else 'Simulation Stopped'}
...
"""
            ...
            self.log_message(f"   Input fields locked: {'YES' if self.is_running.get() else 'NO'}")
    ...
    # === CORE SIMULATION METHODS (keeping the rest for brevity) ===
...
{'='*60}
Enhanced Activity Simulator Pro - Input Management Edition
...
"""
    ...
    def update_display(self):
        ...
        try:
            ...
            # Add input field status
            if self.is_running.get():
                stats_text += " | 🔒 Fields Locked"
    ...
    def refresh_monitoring_display(self):
        ...
        try:
            ...
            # Input field status
            input_status = "🔓 UNLOCKED" if not self.is_running.get() else "🔒 LOCKED"
            
            report = f"""
📊 ENHANCED ACTIVITY SIMULATOR - INPUT MANAGEMENT EDITION
{'='*70}
...
🔄 CURRENT STATUS
   ...
   Input Field Status: {input_status}
...
🔒 INPUT FIELD MANAGEMENT
   Fields Currently: {input_status}
   Locked Reason: {'Simulation is running' if self.is_running.get() else 'Simulation stopped'}
   Total Input Widgets: {len(self.input_widgets)}
   Checkboxes: {len(self.checkbox_widgets)}
   Spinboxes: {len(self.spinbox_widgets)}
   Scales: {len(self.scale_widgets)}
   Comboboxes: {len(self.combobox_widgets)}
...
📊 SESSION STATISTICS
   ...
   Input Lock Events: {'1 (currently locked)' if self.is_running.get() else '0 (currently unlocked)'}
...
"""
    ...
    def _get_performance_recommendation(self):
        ...
        if self.is_running.get():
            return "🔒 Settings locked - stop simulation to modify parameters"
        elif self.file_recovery_count > 5:
    ...
    def save_daily_report(self):
        ...
        try:
            ...
            with open(filename, 'w') as f:
                ...
                f.write(f"Input Field Management: {'Enabled' if len(self.input_widgets) > 0 else 'Disabled'}\n")
                ...
                f.write(f"Input Widgets Managed: {len(self.input_widgets)}\n")
                ...
                if len(self.input_widgets) > 0: features.append("Input Field Management")
                ...
                f.write(f"Input Management: {'ACTIVE' if len(self.input_widgets) > 0 else 'INACTIVE'}\n")
    ...
    def reset_daily_stats(self):
        ...
        if messagebox.askyesno("Reset Stats", "Are you sure you want to reset daily statistics?"):
            ...
            self.update_display()
            self.refresh_monitoring_display()
            self.log_message("🔄 Daily statistics reset (including recovery stats and input management)")
    ...
    def save_logs(self):
        ...
        try:
            ...
            with open(filename, 'w') as f:
                ...
                f.write(f"Auto-Activation Enabled: {'Yes' if self.auto_activate_enabled.get() else 'No'}\n")
                f.write(f"Auto-Recovery Enabled: {'Yes' if self.auto_file_recovery.get() else 'No'}\n")
                f.write(f"Input Field Management: {'Yes' if len(self.input_widgets) > 0 else 'No'}\n")
                f.write(f"Lock Screen Mode: {'Yes' if self.work_when_locked.get() else 'No'}\n")
                f.write(f"Mouse Movement Type: {self.mouse_movement_type.get()}\n")
                f.write(f"File Recovery Count: {self.file_recovery_count}\n")
                f.write(f"Backup Files: {len(self.backup_file_paths)}\n")
                f.write(f"Input Widgets: {len(self.input_widgets)}\n")
                f.write(f"Current Input State: {'Locked' if self.is_running.get() else 'Unlocked'}\n\n")
